...
class Life {
  ...
  update(foods: Food[]): void {
    ...
    if (this.restTimer <= 0 && Math.random() < LIFE_CONFIG.REST_CHANCE) {
      this.restTimer = Math.floor(getRandom(LIFE_CONFIG.REST_MIN_DURATION, LIFE_CONFIG.REST_MAX_DURATION));
    }
...
function processMating(): void {
  for (let i = 0; i < lives.length; i++) {
    for (let j = i + 1; j < lives.length; j++) {
      ...
      if (a.hunger > LIFE_CONFIG.HUNGER_THRESHOLD_FOR_MATING || 
          b.hunger > LIFE_CONFIG.HUNGER_THRESHOLD_FOR_MATING) continue;

      if ((a.sex === "male" && b.sex === "female") || 
          (a.sex === "female" && b.sex === "male")) {
        const female = a.sex === "female" ? a : b;
        if (!female.isPregnant && 
            a.position.distanceTo(b.position) < LIFE_CONFIG.MATING_DISTANCE) {
