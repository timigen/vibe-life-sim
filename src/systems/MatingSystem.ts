  ...
  update(deltaTime: number): void {
    for (let i = 0; i < this.entities.length; i++) {
      for (let j = i + 1; j < this.entities.length; j++) {
        ...
        if (aLife.hunger > LIFE_CONFIG.HUNGER_THRESHOLD_FOR_MATING ||
            bLife.hunger > LIFE_CONFIG.HUNGER_THRESHOLD_FOR_MATING) continue;

        if ((aLife.sex === "male" && bLife.sex === "female") ||
